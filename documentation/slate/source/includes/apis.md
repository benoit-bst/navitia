...
---------|--------------------|------------------|--------------------------------------|-----------------
yes      | q                  | string           | The search term                      |
no       | depth              | int              | Json response [depth](#depth)        | 1
no       | type[]             | array of string  | Type of objects you want to query It takes one the following values: [`network`, `commercial_mode`, `line`, `route`, `stop_area`] | [`network`, `commercial_mode`, `line`, `route`, `stop_area`]
no       | disable_disruption | boolean          | Remove disruptions from the response | False
...
---------|-----------------|-----------------|-------------------------------------------------------------------|-------------------
yes      | q               | string          | The search term                                                   |
no       | type[]          | array of string | Type of objects you want to query It takes one the following values: [`stop_area`, `address`, `administrative_region`, `poi`, `stop_point`] | [`stop_area`, `address`, `poi`, `administrative_region`] |
no       | depth           | int             | Json response [depth](#depth)                                     | 1
no       | admin_uri[]     | array of string | If filled, it will filter the search within the given admin urisi |
no       | disable_geojson | boolean         | remove geojson from the response                                  | False
no       | from | string   | Coordinates longitude;latitude used to prioritize the objects around this coordinate. Note this parameter will be taken into account only if the autocomplete's backend can handle it |
...
Required | name               | Type            | Description                                                         | Default value
---------|--------------------|-----------------|---------------------------------------------------------------------|---------------------------------------------------------------
no       | distance           | int             | Distance range in meters                                            | 500
no       | type[]             | array of string | Type of objects you want to query                                   | [`stop_area`, `stop_point`, `poi`, `administrative_region`]
no       | admin_uri[]        | array of string | If filled, will filter the search within the given admin uris       |
no       | filter             | string          | Use to filter returned objects. for example: places_type.id=theater |
no       | depth              | int             | Json response [depth](#depth)                                       | 1
no       | disable_geojson    | boolean         | Remove geojson from the response                                    | False
no       | disable_disruption | boolean         | Remove disruptions from the response                                | False
no       | count              | int             | Elements per page                                                   | 10
no       | start_page         | int             | The page number (cf the [paging section](#paging))                  | 0
no       | add_poi_infos[]    | enum            | Activate the output of additional infomations about the poi. For example, parking availability (BSS, car parking etc.) in the pois of response. Pass `add_poi_infos[]=none&` or `add_poi_infos[]=&` (empty string) to deactivate all.   | [`bss_stands`, `car_park`]
...
Required | Name                    | Type                            | Description                                                                                           | Default value
---------|-------------------------|---------------------------------|-------------------------------------------------------------------------------------------------------|--------------
no       | from                    | id                              | The id of the departure of your journey. Required to compute isochrones "departure after"             |
no       | to                      | id                              | The id of the arrival of your journey. Required to compute isochrones "arrival before"                |
yes      | datetime                | [iso-date-time](#iso-date-time) | Date and time to go                                                                                   |
yes      | boundary_duration[]     | int                             | A duration delineating a reachable area (in seconds). Using multiple boundary makes map more readable |
no       | forbidden_uris[]        | id                              | If you want to avoid lines, modes, networks, etc.</br> Note: the forbidden_uris[] concern only the public transport objects. You can't for example forbid the use of the bike with them, you have to set the fallback modes for this (`first_section_mode[]` and `last_section_mode[]`) |
no       | first_section_mode[]    | array of string                 | Force the first section mode if the first section is not a public transport one. It takes one the following values: `walking`, `car`, `bike`, `bss`.<br>`bss` stands for bike sharing system.<br>It's an array, you can give multiple modes.<br><br>Note: choosing `bss` implicitly allows the `walking` mode since you might have to walk to the bss station.<br> Note 2: The parameter is inclusive, not exclusive, so if you want to forbid a mode, you need to add all the other modes.<br> Eg: If you never want to use a `car`, you need: `first_section_mode[]=walking&first_section_mode[]=bss&first_section_mode[]=bike&last_section_mode[]=walking&last_section_mode[]=bss&last_section_mode[]=bike` | walking |
no       | last_section_mode[]     | array of string                 | Same as first_section_mode but for the last section                                                   | walking
...
Required | Name               | Type                            | Description                                                                              | Default Value
---------|--------------------|---------------------------------|------------------------------------------------------------------------------------------|--------------
no       | from_datetime      | [iso-date-time](#iso-date-time) | The date_time from which you want the schedules                                          |
no       | duration           | int                             | Maximum duration in seconds between from_datetime and the retrieved datetimes.           | 86400
no       | depth              | int                             | Json response [depth](#depth)                                                            | 1
no       | items_per_schedule | int                             | Maximum number of columns per schedule.                                                  |
no       | forbidden_uris[]   | id                              | If you want to avoid lines, modes, networks, etc.                                        |
no       | data_freshness     | enum                            | Define the freshness of data to use<br><ul><li>realtime</li><li>base_schedule</li></ul>  | base_schedule
...
no       | depth              | int                             | Json response [depth](#depth)                                                                            | 1
no       | forbidden_uris[]   | id                              | If you want to avoid lines, modes, networks, etc.                                                        |
no       | items_per_schedule | int                             | Maximum number of datetimes per schedule.                                                                |
no       | data_freshness     | enum                            | Define the freshness of data to use to compute journeys <ul><li>realtime</li><li>base_schedule</li></ul> | base_schedule
no       | disable_geojson    | boolean                         | remove geojson fields from the response                                                                  | False
...
Required | Name             | Type                            | Description                                                                                              | Default Value
---------|------------------|---------------------------------|----------------------------------------------------------------------------------------------------------|--------------
no       | from_datetime    | [iso-date-time](#iso-date-time) | The date_time from which you want the schedules                                                          | the current datetime
no       | duration         | int                             | Maximum duration in seconds between from_datetime and the retrieved datetimes.                           | 86400
no       | count            | int                             | Maximum number of results.                                                                               | 10
no       | depth            | int                             | Json response [depth](#depth)                                                                            | 1
no       | forbidden_uris[] | id                              | If you want to avoid lines, modes, networks, etc.                                                        |
no       | data_freshness   | enum                            | Define the freshness of data to use to compute journeys <ul><li>realtime</li><li>base_schedule</li></ul> | realtime
no       | disable_geojson  | boolean                         | remove geojson fields from the response                                                                  | false
